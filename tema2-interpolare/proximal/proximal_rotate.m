function R = proximal_rotate(I, rotation_angle)
    ...
    for y = 0 : m - 1
	    ...
	    elseif (x_p <= n && x_p >= 1 && y_p <= m && y_p >= 1)
	    	x1 = floor(x_p); x2 = x1 + 1;
	    	y1 = floor(y_p); y2 = y1 + 1;
            ...
            % conform formulei.
	       cf = proximal_coef(I, x1, y1, x2, y2);
	       final_pixel = cf(1) + cf(2) * x_p + cf(3) * y_p + cf(4) * x_p * y_p;
	    endif
            % TODO: Calculeaza valoarea interpolata a pixelului (x, y).
            R(y+1, x+1) = final_pixel;
        endfor
    endfor
